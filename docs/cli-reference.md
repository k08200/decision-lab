...
node bin/decision-lab.js start "Should we change pricing?" --type business --owner "Your Name" --slug pricing
...
`readiness` writes a commercial readiness report that separates personal, open-source, local commercial, SaaS-transition, and hosted SaaS readiness.
