...
`serve` starts a local product UI with portfolio filtering, report tabs, decision creation, JSON editing, validated saves, memo previews, first-run guidance, operating-loop actions, and JSON/Markdown endpoints for automation.
...
- imports evidence from CSV, TSV, JSON, Markdown, text, saved HTML, PDF, XLSX, and live HTTP(S) URL sources
- extracts evidence from Markdown, text notes, saved web pages, PDFs, spreadsheets, and live URLs with citation metadata
...
- runs a local product UI/API server with decision create/edit/save support and first-run guidance
