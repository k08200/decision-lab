...
const VALID_TYPES = new Set(["general", "investment", "business", "finance"]);
const VALID_ROLES = new Set(["analyst", "skeptic", "cfo", "ceo", "recorder"]);
...
export function scoreDecision(decision) {
  const checks = [
    check("clear_question", hasText(decision.question), 10, "Decision question is explicit."),
    check("context", hasText(decision.context), 10, "Context explains why this decision matters now."),
    check("hypothesis_depth", minLength(decision.hypotheses, 2), 10, "At least two hypotheses are stated."),
    check("evidence", minLength(decision.evidence, 3), 15, "At least three evidence items are attached."),
    check("counterarguments", everyHypothesisHas(decision, "counterarguments"), 15, "Each hypothesis has counterarguments."),
    check("disconfirming_signals", everyHypothesisHas(decision, "disconfirming_signals"), 10, "Each hypothesis has falsification signals."),
    check("criteria", minLength(decision.decision_criteria, 3), 10, "Decision criteria are explicit."),
    check("change_mind", minLength(decision.what_would_change_my_mind, 2), 10, "Change-my-mind conditions are stated."),
    check("review_loop", hasReviewLoop(decision), 10, "Review metrics and questions exist.")
  ];
...
export function renderDecisionMemo(decision) {
  ...
  const sections = [
    ...
    "## Decision Criteria",
    bulletList(decision.decision_criteria || []),
...
export function buildRolePrompt(role, decision) {
  ...
  const roleInstructions = {
    analyst: "You are the Analyst. Strengthen the thesis, identify missing evidence, separate facts from assumptions, and propose the next research tasks.",
    skeptic: "You are the Skeptic. Attack the conclusion, find base-rate mistakes, expose hidden assumptions, and list disconfirming evidence that would break the thesis.",
    cfo: "You are the CFO. Translate the decision into financial impact, cash risk, opportunity cost, downside protection, and capital allocation tradeoffs.",
    ceo: "You are the CEO. Judge strategic fit, timing, execution capacity, stakeholder impact, and whether this decision compounds the organization's advantage.",
    recorder: "You are the Decision Recorder. Produce a concise decision memo, log assumptions, define review metrics, and make the final recommendation auditable."
  };
...
- Mark claims as fact, inference, or assumption.
- Add counterarguments before strengthening the recommendation.
- Return concrete edits that can be merged back into the JSON record.
- If evidence is missing, state exactly what evidence is needed.
...
function renderOption(option) {
  return `${option.id}: ${option.name} - ${option.description} Upside: ${option.upside || "N/A"} Downside: ${option.downside || "N/A"}`;
}
...
function renderEvidence(evidence) {
  return `${evidence.claim} Source: ${evidence.source}. Strength: ${evidence.strength}. Notes: ${evidence.notes || "N/A"}`;
}

function renderRisk(risk) {
  return `${risk.risk} Probability: ${risk.probability || "N/A"}. Impact: ${risk.impact || "N/A"}. Mitigation: ${risk.mitigation}`;
}
